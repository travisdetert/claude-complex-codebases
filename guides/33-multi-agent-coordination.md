...
```mermaid
flowchart TD
    A[Should I use<br/>multiple agents?] --> B{Are the subtasks<br/>truly independent?}
    B -->|No, they share files<br/>or depend on each other| C[Use a single session.<br/>Multiple agents will<br/>create conflicts.]
    B -->|Yes, different files<br/>or repos| D{Is the task<br/>large enough?}
    D -->|Small — would take<br/>< 15 min in one session| E[Use a single session.<br/>Overhead isn't worth it.]
    D -->|Large — multiple<br/>components or repos| F{Do you need<br/>speed or isolation?}
    F -->|Speed — do it<br/>all at once| G[Parallel agents<br/>with worktrees]
    F -->|Isolation — separate<br/>PRs for review| H[Separate sessions,<br/>separate branches]
...
```mermaid
flowchart TD
    A[Shared types<br/>and interfaces] --> B[API endpoints]
