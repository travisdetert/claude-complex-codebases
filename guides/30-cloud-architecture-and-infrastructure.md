...
```mermaid
flowchart TD
    A[What are you<br/>building?] --> B{Team size<br/>and complexity?}
    B -->|Small team,<br/>simple app| C[Monolith on<br/>managed services]
    B -->|Growing team,<br/>some complexity| D[Modular monolith<br/>with managed DB]
    B -->|Multiple teams,<br/>high complexity| E[Microservices<br/>with orchestration]
    B -->|Event-driven,<br/>intermittent traffic| F[Serverless /<br/>Functions]
...
```mermaid
flowchart TD
    Internet[Internet] --> ALB[Load Balancer<br/>Allow: 80, 443<br/>From: 0.0.0.0/0]
    ALB --> App[App Servers<br/>Allow: 8080<br/>From: ALB only]
    App --> DB[Database<br/>Allow: 5432<br/>From: App SG only]
    App --> Cache[Redis<br/>Allow: 6379<br/>From: App SG only]
...
flowchart LR
    subgraph Observability
        A[Logs<br/>What happened?] --- B[Metrics<br/>How much / how fast?]
        B --- C[Traces<br/>Where did time go?]
    end

    A --> D[CloudWatch / Datadog /<br/>ELK Stack]
    B --> E[Prometheus + Grafana /<br/>CloudWatch Metrics]
    C --> F[Jaeger / X-Ray /<br/>Datadog APM]
