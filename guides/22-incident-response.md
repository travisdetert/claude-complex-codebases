...
```mermaid
flowchart LR
    A["Detect<br/>Alert fires,<br/>user report"] --> B["Triage<br/>Severity,<br/>impact scope"]
    B --> C["Mitigate<br/>Stop the<br/>bleeding"]
    C --> D["Diagnose<br/>Find root<br/>cause"]
    D --> E["Resolve<br/>Fix and<br/>verify"]
    E --> F["Postmortem<br/>Learn and<br/>prevent"]
...
```mermaid
flowchart TD
    A[Production<br/>Incident] --> B{Caused by<br/>recent deploy?}
    B -->|No/Unclear| C["Investigate —<br/>check infra,<br/>dependencies,<br/>traffic"]
    B -->|Yes| D{Can you<br/>roll back?}
    D -->|No — database<br/>migration| E{Is the migration<br/>backward<br/>compatible?}
    E -->|Yes| F["Roll back code,<br/>leave migration"]
    E -->|No| G["Fix forward —<br/>deploy hotfix"]
    D -->|Yes| H{Impact if you<br/>roll back?}
    H -->|"Low — just<br/>reverts feature"| I["Roll back<br/>immediately"]
    H -->|"High — loses<br/>important changes"| J{Can you fix<br/>in < 30 min?}
    J -->|Yes| K["Fix forward"]
    J -->|No| I
...
```mermaid
flowchart TD
    A["Problem:<br/>Production 502 errors<br/>after deploy"] --> B["Why?<br/>Database connections<br/>exhausted"]
    B --> C["Why?<br/>Migration locked a<br/>table for 4 minutes"]
    C --> D["Why?<br/>Migration added a<br/>column with DEFAULT<br/>on a 10M row table"]
    D --> E["Why?<br/>No migration review<br/>process for<br/>large tables"]
    E --> F["Why?<br/>We didn't have a<br/>migration safety<br/>checklist"]
