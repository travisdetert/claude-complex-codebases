...
```mermaid
flowchart LR
    A["Requirements<br/>What data do<br/>we need?"] --> B["Entities<br/>What are the<br/>things?"]
    B --> C["Relationships<br/>How do they<br/>connect?"]
    C --> D["Normalization<br/>Eliminate<br/>redundancy"]
    D --> E["Indexing<br/>Optimize for<br/>queries"]
    E --> F["Migration<br/>Ship it<br/>safely"]
...
```mermaid
flowchart TD
    A[Normalized<br/>Schema] --> B{Performance<br/>problem?}
    B -->|No| C["Stay normalized<br/>Correct by default"]
    B -->|Yes| D{Measured with<br/>EXPLAIN?}
    D -->|No| E["Measure first<br/>Don't guess"]
    D -->|Yes| F{Can an index<br/>fix it?}
    F -->|Yes| G["Add index<br/>Cheaper than<br/>denormalizing"]
    F -->|No| H{Read-heavy or<br/>write-heavy?}
    H -->|Write-heavy| I["Consider write<br/>optimization<br/>(queues, batching)"]
    H -->|Read-heavy| J["Denormalize<br/>with a plan<br/>to keep in sync"]
...
```mermaid
flowchart TD
    A[Slow Query] --> B{Run<br/>EXPLAIN ANALYZE}
    B --> C{Sequential scan<br/>on large table?}
    C -->|Yes| D{Missing index?}
    D -->|Yes| E["Add index"]
    D -->|No| F{Filter too broad?}
    F -->|Yes| G["Narrow WHERE<br/>clause or add<br/>partial index"]
    F -->|No| H["Table may need<br/>partitioning"]
    C -->|No| I{Nested loops<br/>with high rows?}
    I -->|Yes| J["Check for<br/>N+1 queries<br/>or missing join index"]
    I -->|No| K{Sort/Hash<br/>taking long?}
    K -->|Yes| L["Add index matching<br/>ORDER BY or<br/>increase work_mem"]
    K -->|No| M["Check for<br/>unnecessary columns,<br/>subquery vs join"]
