...
```mermaid
flowchart TD
    A["Accessibility<br/>Improvements"] --> B["Permanent<br/>Disabilities"]
    A --> C["Temporary<br/>Conditions"]
    A --> D["Situational<br/>Limitations"]
    A --> E["Everyone"]

    B --> B1["Blind, deaf,<br/>motor impairments"]
    C --> C1["Broken arm,<br/>ear infection,<br/>concussion"]
    D --> D1["Bright sunlight,<br/>noisy room,<br/>holding a baby"]
    E --> E1["Better SEO,<br/>faster navigation,<br/>mobile usability"]
...
```mermaid
flowchart TD
    A["User clicks<br/>'Open Modal'"] --> B["Modal opens"]
    B --> C["Focus moves to<br/>first focusable<br/>element in modal"]
    C --> D["Tab cycles within<br/>modal only<br/>(focus trap)"]
    D --> E["User closes modal<br/>(Esc or close button)"]
    E --> F["Focus returns to<br/>the element that<br/>opened the modal"]
