...
```mermaid
flowchart TD
    PM["Product Manager<br/>Business requirements,<br/>priorities, timelines"] --> Claude["Claude Code<br/>(Shared codebase access)"]
    Designer["Designer<br/>Components, specs,<br/>visual consistency"] --> Claude
    Engineer["Engineer<br/>Architecture, code,<br/>technical constraints"] --> Claude
    QA["QA Engineer<br/>Test cases, edge cases,<br/>failure modes"] --> Claude
    Analyst["Data Analyst<br/>Schemas, queries,<br/>data pipelines"] --> Claude

    Claude --> Codebase["Actual Codebase<br/>Single source of truth"]

    Claude -->|"Feasibility<br/>analysis"| PM
    Claude -->|"Component<br/>audit"| Designer
    Claude -->|"Impact<br/>analysis"| Engineer
    Claude -->|"Code path<br/>analysis"| QA
    Claude -->|"Schema<br/>mapping"| Analyst

    style Claude fill:#e3f2fd,stroke:#1565c0,stroke-width:3px
    style Codebase fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
```
...
sequenceDiagram
    ...
    PM->>Claude: "We want users to filter<br/>dashboard widgets by date range"
    Claude->>Claude: Analyzes dashboard code,<br/>widget system, data layer
    Claude->>PM: "The widget system loads data<br/>via WidgetDataService. 4 of 7<br/>widgets support date params.<br/>3 would need changes."
    Claude->>Eng: "The 3 widgets without date<br/>support use aggregated caches.<br/>Adding date filtering means<br/>bypassing the cache or<br/>building date-aware caching."
    Eng->>PM: "The caching issue makes this<br/>medium scope, not small.<br/>We could ship the 4 easy<br/>widgets first in one sprint."
    PM->>Eng: "Let's do that. Phase 1:<br/>4 widgets. Phase 2: the<br/>cached ones next sprint."
```
