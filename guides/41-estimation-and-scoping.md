...
```mermaid
flowchart TD
    A[Feature Request] --> B{How well do<br/>you understand<br/>the codebase area?}
    B -->|Well| C[Ask Claude to<br/>analyze blast radius]
    B -->|Not well| D[Run a spike first]
    D --> C
    C --> E[Decompose into subtasks]
    E --> F[Estimate each subtask<br/>with three-point range]
    F --> G[Add integration time<br/>between components]
    G --> H[Add risk buffer based<br/>on unknowns]
    H --> I[Present range<br/>to stakeholders]
    I --> J[Track actual vs estimated]
    J --> K[Calibrate future<br/>estimates]
...
```mermaid
flowchart TD
    A[New Task] --> B{Have you done<br/>something very similar<br/>in this codebase before?}
    B -->|Yes| C{Is it isolated —<br/>no cross-cutting<br/>concerns?}
    C -->|Yes| D[Quick estimate:<br/>T-shirt size is fine]
    C -->|No| E[Standard estimate:<br/>decompose + ranges]
    B -->|No| F{Do you understand<br/>the relevant code<br/>well enough to<br/>identify subtasks?}
    F -->|Yes| E
    F -->|No| G{Is the task<br/>expected to take<br/>more than 3 days?}
    G -->|No| H[Time-box a spike:<br/>half day max]
    G -->|Yes| I[Full spike + detailed<br/>decomposition + risk analysis]
