...
```mermaid
flowchart TD
    A[Need to change<br/>the API?] --> B{Is it a<br/>breaking change?}
    B -->|No| C[Ship it —<br/>no version needed]
    B -->|Yes| D{Can you make it<br/>non-breaking?}
    D -->|Yes| E[Make it additive:<br/>new fields, new<br/>endpoints]
    D -->|No| F{How many<br/>consumers?}
    F -->|"Few (internal)"| G[Coordinate and<br/>migrate together]
    F -->|"Many (external)"| H[Create new<br/>version]
...
```mermaid
flowchart LR
    A["Announce<br/>Document the<br/>deprecation"] --> B["Warn<br/>Return deprecation<br/>headers/logs"]
    B --> C["Sunset<br/>Return errors for<br/>new integrations"]
    C --> D["Remove<br/>Endpoint returns<br/>410 Gone"]
