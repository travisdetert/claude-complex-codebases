...
```mermaid
flowchart LR
    A[Understand the<br/>Workload] --> B[Estimate<br/>Resources]
    B --> C[Model<br/>Costs]
    C --> D[Provision<br/>Infrastructure]
...
```mermaid
flowchart TD
    A[Need More<br/>Capacity] --> B{What's the<br/>bottleneck?}
    B -->|CPU| C{Can the workload<br/>be parallelized?}
    C -->|Yes| D[Scale Horizontally<br/>Add more instances]
    C -->|No| E[Scale Vertically<br/>Bigger instance]
    B -->|Memory| F{Is it cache<br/>or working set?}
    F -->|Cache| G[Scale Horizontally<br/>Distributed cache]
    F -->|Working set| H[Scale Vertically<br/>More RAM]
    B -->|Database| I{Read or<br/>write bound?}
    I -->|Read| J[Add Read Replicas]
    I -->|Write| K[Scale Vertically<br/>or Shard]
    B -->|Network| L[Add CDN /<br/>Edge Caching]
