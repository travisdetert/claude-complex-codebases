...
```mermaid
flowchart TD
    subgraph "Service A (Works Perfectly)"
        A1[Business Logic] --> A2[API Client]
    end

    subgraph "The Danger Zone"
        A2 -->|HTTP / gRPC / Queue| B1
        style A2 fill:#ffebee,stroke:#c62828
        style B1 fill:#ffebee,stroke:#c62828
    end

    subgraph "Service B (Works Perfectly)"
        B1[API Endpoint] --> B2[Business Logic]
    end
...
flowchart TD
    ...
    linkStyle 4,5 stroke:#1565c0
    linkStyle 6,9,10 stroke:#e65100
    linkStyle 7 stroke:#c62828
```
...
```mermaid
flowchart LR
    subgraph "Provider (Order Service)"
        P[API Endpoint<br/>POST /orders]
    end
